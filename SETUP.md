...
### 다른 네트워크(회사 등)에서 데모할 때 — 서버 주소 직접 변경

Tizen 앱에는 집 Mac mini의 고정 LAN IP(`192.168.0.77`)가 기본값으로 박혀 있어서, **다른 Wi-Fi에서는
연결이 안 되는 게 정상**입니다 (IP가 화면에 보여도 그냥 응답이 없는 사설 IP라서). 이전에는 이 상태에서
사용자가 고칠 방법이 없었는데, 이제 리모컨으로 그 자리에서 바꿀 수 있습니다.

1. 연결 중/실패 화면(오버레이)에서 확인(OK)을 누르면 서버 주소 입력창이 뜨고 OSK가 자동으로 열림
2. 데모 당일 Mac mini에서 띄운 Cloudflare Tunnel 주소를 입력 (예: `abc-xyz-handheld.trycloudflare.com`)
   — `http(s)://`는 안 붙여도 됨, 도메인처럼 보이면 자동으로 `wss://`를, IP:포트 형태면 `ws://`를 붙임
3. 확인(OK) → 즉시 그 주소로 재연결 시도. 입력값은 기기에 저장되어 앱을 껐다 켜도 유지됨
4. 다시 집에서 쓸 때는 같은 화면에서 입력칸을 **비우고** 확인 → 기본 주소(고정 LAN IP)로 초기화

즉, 사무실 데모 당일 절차는: ① Mac mini에서 `cloudflared tunnel --config ... --url http://localhost:3000`
실행 → ② 출력된 주소를 TV 리모컨으로 입력 → ③ 끝. 앱을 다시 빌드/서명할 필요가 없습니다.
